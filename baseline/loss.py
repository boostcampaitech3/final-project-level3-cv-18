...
class SoftCrossEntropyLoss(nn.Module):
    ...
    def forward(self, pred, target):
        result = -torch.sum(torch.log10(pred)*target)
        return result
...
class CB_loss(nn.Module):
    def __init__(self,beta=0.9999,gamma=2.0,epsilon=0.1):
    ...
    def forward(self,logits, labels,loss_type = 'focal'):
        ...
        labels_one_hot  = torch.zeros(len(labels), no_of_classes).cuda().scatter_(1, labels, 1)
...
_criterion_entrypoints = {
    ...
    'f1': F1Loss,
    'softmax_ce' : SoftCrossEntropyLoss,
    'CB' : CB_loss
}
...
def eval_criterion(y_pred, y_true,epsilon=1e-7,num_classes=5):
        assert y_pred.ndim == 2
        assert y_true.ndim == 1
